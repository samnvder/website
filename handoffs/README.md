...
| 6 | **[fix-book-tour-double-booking](fix-book-tour-double-booking.md)** | 🔴 **`book-tour` accepts double-bookings.** Slot conflict is enforced only in browser JS; the server accepts a slot `check-availability` reports as taken. Two prospects can land on one tour via simultaneous submits or any non-widget request. Confirmed in production 2026-08-18. **Blocker: the edge-function source is not in this repo** — retrieve and mirror it first. | ~45 min | needs edge-function source |

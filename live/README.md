...
| [`wpcode/9934-noindex-utility-pages.php`](wpcode/9934-noindex-utility-pages.php) | WPCode snippet **9934**, "SEO - Noindex internal & utility pages" — `wpseo_robots_array` + `wpseo_exclude_from_sitemap_by_post_ids`. **9 IDs** including membership-next-steps **10047**. | ✅ **re-captured from the WPCode Code field 2026-08-25.** The file here before that date was a reformatted copy with a `<?php` header the editor does not have (same class of error as 9951). Live editor is two minified `add_filter` calls, no `<?php`. |
...
| [`wpcode/9951-renamed-page-redirects.php`](wpcode/9951-renamed-page-redirects.php) | WPCode snippet **9951**, "SEO - Renamed-page 301 redirects" — 301s for **four** renamed pages (`special-offer` added 2026-08-19) | ✅ **re-captured from the editor 2026-08-19.** ⚠️ The file here before that date was **not a capture** — a rewrite that was never pasted back, 2,459 bytes against the real 1,465. See [CLAUDE.md](../CLAUDE.md#-capture-by-asking-for-a-paste-not-by-scraping-the-rendered-page) |

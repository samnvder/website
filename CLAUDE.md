...
## ⚠️ The naming law — no referential phrases (owner-mandated 2026-08-25)

When telling the owner to click, copy, or paste, **name the thing**. "The code
box", "that snippet", "the JS file", "the page", "in there", "the editor" are
not instructions. This failed on WPCode **9934**: the owner was asked to copy
"the code box" and did not know which field that was.

Required in every owner-facing step:

- **Exact filename**, including the directory when more than one file could match
  (`Website/Pages/Memberships (Category)/membership-next-steps/Membership Next Steps redirect.js`).
- **Exact WPCode snippet ID and title** (`9934` / `SEO - Noindex internal & utility pages`).
- **Exact admin labels** as they appear on screen: menu names, field names,
  button text (`Update`, notice `"Snippet updated."`).
- **Which editor**: Gutenberg **code editor**, Thrive **Custom CSS**, Thrive
  **Custom HTML**, WPCode **Code** field — never "the editor" alone.
- If a control has no useful label, identify it by **what is already in it**
  (the WPCode **Code** field that starts with `add_filter( 'wpseo_robots_array'`).
- **Clickable local-file links in Cursor chat.** Every owner-facing mention of
  a local file must link directly to that file with Markdown, using the exact
  filename as the link text and the repo-relative path as the target:
  [`Membership Next Steps redirect.js`](Website/Pages/Memberships%20%28Category%29/membership-next-steps/Membership%20Next%20Steps%20redirect.js).
  Never provide only a plain-text or backticked file path. If paste delivery
  also requires a parent-folder path for Explorer, provide both the folder
  block and the clickable filename.
- **Deterministic multi-component delivery.** When several components or
  different code blocks must be applied, output one numbered section per live
  destination. Every section follows the same order: destination title,
  clickable file link, parent-folder block, exact admin navigation, exact
  editor or element identifier, replace/paste action, exact save button and
  success signal, then character-count delta when available. End with cache
  flush and verification. Do not collapse destinations into a summary table
  or vague prose, and do not open files in Notepad or another external editor.
  The canonical example is
  [`.claude/skills/deliver-paste/GOLD-STANDARD.md`](./.claude/skills/deliver-paste/GOLD-STANDARD.md).

Handoffs, paste instructions, and chat with the owner all follow this. Vague
nouns are a defect, not shorthand.

## ⚠️ These rules are mirrored — edit CLAUDE.md, never the copies

**This file is the source.** Cursor, Copilot and every other agent read generated copies of it, so that
each tool gets the *same* ruleset rather than a drifting variant:

| File | Read by |
|---|---|
| [`AGENTS.md`](./AGENTS.md) | Codex, Amp, Jules, most CLI agents |
| [`.cursor/rules/southend.mdc`](./.cursor/rules/southend.mdc) | Cursor |
...
`guard:agent-rules` is in the `guard` chain, so drift fails `npm run guard`. It tells the two cases apart:
...
## ⚠️ The paste law — paste from the page folder, never from `patches/`

**You edit and paste files under `Website/Pages/…`.** `patches/` is not a second
HTML. That split is what made two HTML files and a stale Gutenberg paste.

New page HTML is **Gutenberg** (`<!-- wp:html -->` in the page `* HTML.html`).
A **Thrive source** sibling is generated from that file, never edited by hand:

```powershell
npm run convert:thrive-source
```

`npm run guard:thrive-source` fails if the sibling is missing or drifted.
Paste the Gutenberg file into Gutenberg. Paste `* thrive-source.html` into a
Thrive **Custom HTML** element only if Gutenberg is refused. Never paste into
Thrive **View Page Source**.

If a destination cannot accept the source as-is (post content cannot contain
`<script>` because `&&` is entity-encoded, Thrive `wpautop`s a `<style>` block
into `<p>` tags): **split the source into paste-ready pieces in the same
directory** (markup file, CSS file, JS file). The Thrive source file is the one
allowed generated sibling — produced by the converter, not by hand.

| Paste | File you open |
|---|---|
| HTML (Gutenberg) | the page `* HTML.html` |
| HTML (Thrive Custom HTML, fallback) | the page `* thrive-source.html` (generated) |
| CSS | the page `* CSS.css` (Thrive Custom CSS, or WPCode CSS) |
| Page JS | the page `* JS.js` (WPCode) |
| Site-wide helper JS | authored next to the page, same folder |

`patches/` holds instructions, GTM notes, campaign packs, and authored one-offs
that are **not** a second copy of a page file. Campaign `PAGE--` / `HOME--` /
`WPCODE--` packs are generated *from* the campaign engine writing the page
files — they are not a parallel source.

**Still true:** never paste a *lagging whole-page* file (memberships, homepage,
special-offer) over a live Thrive page. The repo lags live; that paste deletes
whatever live has and the repo doesn't. This law is about not duplicating a
page we built here to be pasted.
...
- **The naming law.** Every human step names the file, snippet ID + title, and
  on-screen field. No "the code box" / "that snippet" / "the editor".
...
- **Paste from the page folder**, never a `patches/` copy of the HTML (see **The paste law**). Gutenberg `* HTML.html` is the file you edit; `* thrive-source.html` is generated (`npm run convert:thrive-source`). `patches/<task>/` is for instructions, GTM notes, and authored one-offs.
...
**Never paste a lagging whole-page file over live Thrive.** Homepage, `/memberships/`, `/special-offer/` lag live (e.g. `se-bk-floating` ran for months with no repo copy). Pasting those over a live page silently deletes whatever live has and the repo doesn't. Utility pages built here to be pasted (this law) are the opposite: paste those files.
...
- ~~**`npm run guard` is broken on `master`**~~ — **fixed 2026-08-18.** `npm run guard` exits 0, so **a red check now means something.** History, because the wrong fix here is tempting:
  ...
  **Because the templates are reused, stale values are a launch hazard, not dead text.** Publishing an offer page activates whatever the snippet holds — including the `offer:` tag that reaches Heroku and Dropbox Sign, which would mislabel every signup of a *new* campaign as the old one. #7966 now rests carrying **no offer at all**: its wording and tag are loud placeholders (`OFFER NOT SET`, `UNSET-set-before-launch`), so an accidental publish fails obviously instead of plausibly. Each mirror's header carries its pre-launch checklist, and `guard:stale-offer` enforces the dates.
  ...
  - **#7966's `{1: 25, 2: 15}` was an oversight — owner-confirmed — and is now `{1: 30, 2: 20}` in all three WPCode builders: repo, live and mirror.** Pasted into WPCode 2026-08-18 via [`patches/fix-7966-young-discounts/`](./patches/fix-7966-young-discounts/), which also neutralised the expired campaign. ⚠️ **"Everywhere" was wrong, and so was "all three builders" — there is a **fourth** copy.** `/special-offer/` does not use a WPCode snippet at all: it **inlines its own builder** in the page HTML (its own comment says *"do not rely on WPCode 7966 for this page"*), and that copy carried `{1: 25, 2: 15}` plus the expired `summer-special-2026-jul31` tag, where neither guard could see it. **Both halves are now fixed — verified 2026-08-19.** `8c96254` parked the campaign in the page source (line 2104 reads `{ 1: 30, 2: 20 }`, line 2265 reads `offer: "UNSET-set-before-launch"`), and `8b72083` widened `guard:stale-offer` past `live/wpcode/*.js` — it now reports **"3 mirrored snippets + 29 page files"**, so the fourth copy is in scope. See [SEO/TODO.md](./SEO/TODO.md) §29.

    ⚠️ **This paragraph asserted the opposite for a while after the fix landed, and so did both generated mirrors.** The fix was committed, the record was not updated, and the stale text was specific enough to be believed. If you are about to act on a ⚠️ here, check the commit it names still reflects the file.
...
- **`npm run guard:capture-markup` is in the `guard` chain and green (added 2026-08-20, owner-requested).** It runs the capture converter in `--check` mode over `Website/Pages` and fails if any paste-source page file carries output-only markup (CompressX `<picture>`/`<source>` wrappers, `tve_js_placeholder`, expanded boolean attributes). This makes rule 4 of the backup law *mechanical* for page trees: converted output cannot get into repo sources undetected. The converter's rules are still a whitelist of observed shapes, so the full deterministic guarantee remains the pair *converter + `--diff` against a real capture* — the guard enforces the half that can run unattended.

  `live/thrive` is deliberately **out of scope**: its three mirrors currently carry 16 expanded boolean attributes, and whether the live editors genuinely hold them is unknowable without a paste from each editor. "Cleaning" the mirrors to make a check green would fabricate them — the exact 9951 failure. Widen the guard's scope only after re-capturing those mirrors by paste.
